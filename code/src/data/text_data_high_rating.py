...
#modified
def summary_merge_high_rate(df, user_id, max_summary):
    """
    Parameters
    ----------
    df : pd.DataFrame
    ...
    """
    #summary_select = df[df['user_id'] == user_id].sort_values(by='summary_length', ascending=False)['summary'].values[:max_summary]
...
def process_text_data(df, books, user2idx, isbn2idx, device, train=False, user_summary_merge_vector=False, item_summary_vector=False):
    ...
    if user_summary_merge_vector and item_summary_vector:
        ...
        for user in tqdm(df_['user_id'].unique()):
            #modified
            vector = text_to_vector(summary_merge_high_rate(df_, user, 5), tokenizer, model, device)
        ...
        if train == True:
            #modified
            np.save('./data/text_vector/train_user_summary_merge_vector_mod.npy', vector)
        else:
            #modified
            np.save('./data/text_vector/test_user_summary_merge_vector_mod.npy', vector)
        ...
        if train == True:
            #modified
            np.save('./data/text_vector/train_item_summary_vector_mod.npy', vector)
        else:
            #modified
            np.save('./data/text_vector/test_item_summary_vector_mod.npy', vector)
    else:
        print('Check Vectorizer')
        print('Vector Load')
        if train == True:
            #modified
            user = np.load('data/text_vector/train_user_summary_merge_vector_mod.npy', allow_pickle=True)
            #user = np.load('data/text_vector/train_user_summary_merge_vector.npy', allow_pickle=True)
        else:
            #modified
            user = np.load('data/text_vector/test_user_summary_merge_vector_mod.npy', allow_pickle=True)
        ...
        if train == True:
            #modified
            item = np.load('data/text_vector/train_item_summary_vector_mod.npy', allow_pickle=True)
            #item = np.load('data/text_vector/train_item_summary_vector.npy', allow_pickle=True)
        else:
            #modified
            item = np.load('data/text_vector/test_item_summary_vector_mod.npy', allow_pickle=True)
