...
from src.data import hybrid_data_load, model_data_split
...
def main(args):
    ...
    ######################## Wandb init
    wandb.init(project = 'Book Recommendation')

    ######################## Wandb run
    wandb.run.name = 'Model experiment'
    wandb.run.save()
    ...
    elif args.model in ('DeepCoNN', 'LSTM_FM'):
        import nltk
        nltk.download('punkt')
    ...
    if args.model in ('FM', 'FFM'):
       ...
       data_cold,data_not_cold = model_data_split(args, data)
    ...
    setting.make_dir('./submit')
    submission.to_csv(filename, index=False)
    
    ######################## Evaluation
    #evaluation('./data/train_ratings.csv', filename)
...
if __name__ == "__main__":
    ...
    arg('--model', type=str, choices=['FM', 'FFM', 'NCF', 'WDN', 'DCN', 'CNN_FM', 'DeepCoNN', 'LSTM_FM', 'Hybrid'],
                                help='학습 및 예측할 모델을 선택할 수 있습니다.')
    arg('--model2', type=str, choices=['FM', 'FFM', 'NCF', 'WDN', 'DCN', 'CNN_FM', 'DeepCoNN', 'LSTM_FM'],
                                help='학습 및 예측할 모델을 선택할 수 있습니다.')
